...
bool sensorEnabled = false;  // Flag untuk mengaktifkan/menonaktifkan sensor jarak (default: AKTIF)
...
void loop() {
  ...
  // --- Pembacaan Sensor Jarak dan Pemicu Buzzer ---
  unsigned long currentMillis = millis();
  // Hanya baca sensor jika diaktifkan dan sudah melewati interval yang ditentukan
  if (sensorEnabled && (currentMillis - lastSensorReadMillis >= sensorReadInterval)) {
  ...
  // Memeriksa apakah argumen "State" ada dalam permintaan HTTP
  if (server.hasArg("State")) {
    ...
    // Menjalankan aksi motor dan robot berdasarkan perintah yang diterima
    ...
    else if (command == "S") Stop();          // Menghentikan motor
...
<style>
  ...
  .button.toggle-light { background-color: #00bcd4; } /* Cyan */
  .button.toggle-light:active { background-color: #008ba3; }
